...
현재 구현은 Next.js 모바일 웹/PWA 런타임을 공유 표면으로 유지하면서 Capacitor-first 내부 테스트 앱 트랙을 준비합니다.
모든 주요 화면은 390px 폭 모바일 프레임에서 먼저 검증하고, UI 변경은 모바일 브라우저와 Capacitor WebView 모두에서 같은 구조로 동작해야 합니다.
...
- PWA manifest, 앱 아이콘, 설치 가능성은 공유 웹 런타임 체크리스트로 관리합니다.
- Capacitor shell 작업은 기본적으로 native permission, push notification, biometric unlock, native secure storage를 추가하지 않습니다. 필요해지면 별도 보안/법무/운영 gate와 모바일 QA 체크리스트를 먼저 만듭니다.
...
- 모바일 브라우저, PWA 설치 상태, 가능한 경우 Capacitor WebView에서 기본 이동 흐름을 확인합니다.
